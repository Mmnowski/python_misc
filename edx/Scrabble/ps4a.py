...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for letter in word:
            if letter not in hand or hand_copy[letter] == 0:
                return False
            else:
                hand_copy[letter] -= 1
        return True
...
def playGame(wordList):
    ...
    while True:
        ...
        if option = 'n':
            lastGame = dealHand(handLength)
            playHand(lastGame, wordList, handLength)
        elif option = 'r':
            if calculateHandlen(lastGame) > 0:
                playHand(lastGame, wordList, calculateHandlen(lastGame))
            else:
